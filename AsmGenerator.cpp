...
void AsmGenerator::generateArithmetic(Quad& q) {
	...
	std::string resultIdName = q.getArg(3).var->getIdName(); // �ҵ�����������
	...
	if (flag == 7) {
		...
		if (tempVar1Reg != asmRegister::unset && tempVar2Reg != asmRegister::unset) {
			...
			std::string var1_ebp_offset = this->asmcode.generateVar(var1->getOffset()); // �õ�ƫ����
			std::string var2_ebp_offset = this->asmcode.generateVar(var2->getOffset()); // �õ�ƫ����
			if (resultReg != asmRegister::unset) {
				if (opcode == OpCode::PLUS || opcode == OpCode::MINUS) {
